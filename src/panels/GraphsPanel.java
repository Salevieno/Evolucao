...
public abstract class GraphsPanel
{
	...
	static
	{
		...
		charts.add(new Chart(new Point(45, 200), "Population", 80, Species.getColors())) ;
		charts.get(0).addDatasets(popData) ;
		charts.add(new Chart(new Point(45, 350), "Eating gene", 80, Species.getColors())) ;
		charts.get(1).addDatasets(geneData) ;
		charts.add(new Chart(new Point(45, 500), "fps", 80, Color.orange)) ;
	...
	public static void updateRecords(List<Artro> artros)
	{
		for (int i = 0 ; i <= Species.getAll().size() - 1 ; i += 1)
		{
			...
			geneData.get(i).addYDataUpToSize(Artro.getAllOfSpecies(species).stream().mapToDouble(artro -> artro.getChoice().get(ArtroChoices.eat)).average().getAsDouble(), 200) ;
		}
