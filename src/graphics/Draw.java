...
public abstract class Draw
{
	...
	public static void DrawImage(Image File, Point pos, float angle, float[] Scale, String Alignment)
	{
		if (File != null)
		{
			...
			{
				a = AffineTransform.getRotateInstance(-angle * Math.PI / 180, pos.x + l / 2, pos.y - h / 2); // Rotate
																												// image
				G.setTransform(a);
	...
	{
		G.drawImage(image, pos.x - image.getWidth(null) / 2, pos.y - image.getHeight(null) / 2, image.getWidth(null),
				image.getHeight(null), null);
	}
	/*
	 * public static void DrawText(Point pos, String Text, String Alignment, float
	 * angle, String Style, int size, Color color) { float TextL = UtilS.TextL(Text,
	 * TextFont, G), TextH = UtilS.TextH(size); int[] Offset =
	 * UtilS.OffsetFromPos(Alignment, (int)TextL, (int)TextH); AffineTransform a =
	 * null; // Rotate rectangle AffineTransform backup = G.getTransform(); if
	 * (Alignment.equals("Left")) { a =
	 * AffineTransform.getRotateInstance(-angle*Math.PI/180, pos.x - 0.5*TextL,
	 * pos.y + 0.5*TextH); // Rotate text } else if (Alignment.equals("Center")) { a
	 * = AffineTransform.getRotateInstance(-angle*Math.PI/180, pos.x, pos.y +
	 * 0.5*TextH); // Rotate text } else if (Alignment.equals("Right")) { a =
	 * AffineTransform.getRotateInstance(-angle*Math.PI/180, pos.x, pos.y +
	 * 0.5*TextH); // Rotate text } if (Style.equals("Bold")) { G.setFont(new
	 * Font(BoldTextFont.getName(), BoldTextFont.getStyle(), size)); } else {
	 * G.setFont(new Font(TextFont.getName(), TextFont.getStyle(), size)); } if (0 <
	 * Math.abs(angle)) { G.setTransform(a); } G.setColor(color); G.drawString(Text,
	 * pos.x + Offset[0], pos.y + Offset[1]); G.setTransform(backup); }
	 */
	/*
	 * public static void DrawFitText(Point Pos, String Text, String Alignment,
	 * float angle, String Style, int sy, int length, int size, Color color) {
	 * String[] FitText = UtilS.FitText(Text, length); for (int i = 0; i <=
	 * FitText.length - 1; i += 1) { DrawText(new int[] {Pos.x, Pos.y + i*sy},
	 * FitText[i], Alignment, angle, Style, size, color); } }
	 */

	public static void DrawRect(Point pos, String alignment, Dimension size, int stroke, Color color,
			Color contourColor)
	{
	...
	public static void DrawRoundRect(Point Pos, String Alignment, int l, int h, int Thickness, int ArcWidth,
			int ArcHeight, Color color, Color ContourColor, boolean contour)
	{
		...
		{
			G.setColor(ContourColor);
			G.fillRoundRect(Pos.x + offset[0] - Thickness, Pos.y + offset[1] - Thickness, l + 2 * Thickness,
					h + 2 * Thickness, ArcWidth, ArcHeight);
		}
		if (color != null)
		{
			G.setColor(color);
	...
	public static void DrawRoundRect(Point Pos, String Alignment, int l, int h, int Thickness, int ArcWidth,
			int ArcHeight, Color[] colors, Color ContourColor, boolean contour)
	{
		int[] offset = UtilS.OffsetFromPos(Alignment, l, h);
		G.setStroke(new BasicStroke(Thickness));
		if (contour)
		...
		{
			GradientPaint gradient = new GradientPaint(Pos.x + offset[0], Pos.y + offset[1], colors[0], offset[0],
					offset[1] + h, colors[1]);
			G.setPaint(gradient);
			G.fillRoundRect(Pos.x + offset[0], Pos.y + offset[1], l, h, ArcWidth, ArcHeight);
		}
	...
	{
		points.forEach(pos -> DP.drawCircle(pos, size, fillColor, contourColor));
	}
